...
```{r}
OSA <- function(train_data, new_data, model) {
  
  # 1. Si hay datos nuevos (X_test_past no está vacío), actualizamos el modelo
  # Arima() (con mayúscula) permite aplicar un modelo existente a nuevos datos
  if(length(new_data) > 0){
     # Unimos los datos históricos con los nuevos para el contexto
     # O simplemente usamos Arima para filtrar los nuevos datos usando el modelo previo
     model <- Arima(c(train_data, new_data), model=model)
  }
  
  # 2. Predecimos
  pred <- forecast(model, h = 1)
  
  # 3. Retornamos vector (Corregimos el error de mu_t[i] que tenías dentro)
  vec <- data.frame(c('mu_hat' = pred$mean, 'se_hat' = mean(pred$model$sigma2)))
...
```{r}
getPred_ts(df_train,df_test,OSA, models)
```
